<template>
  <div id="scoreboard">
    ...
    >
      <img id="back-panel" src="./img/main.svg" />
    </div> -->
    ...
    >
      ...
      >
        <fitty
          class="games-text"
          :options="{ minSize: 1, maxSize: 38, multiLine: false }"
        >
          {{ display.scores[0] }}
        </fitty>
      </div> -->
      ...
      >
        <img
          :src="side_panel_path(0)"
          style="transform: scaleX(-1) skewX(23deg)"
        />
...
<style>
:root {
  --main-panel-height: 30px;
  --main-panel-width: 427.23px;
  ...
  --side-panel-height: calc(var(--name-panel-height) * 1);
  --side-panel-width: 93.35px;
  --side-text-height: var(--side-panel-height);
  --side-text-width: var(--side-panel-width);
  --side-start-x: 1px;
  --side-end-x: calc(var(--side-panel-width) * 1.65 * -1);

  --flag-height: 50px;
  --flag-width: 120px;
  --flag-start-x: calc(var(--flag-width) + 1px);
  --flag-end-x: calc(var(--flag-width) * (1 + 0.01));
